...
def ask_gpt():
    print("____________________\n\nコマンドプロンプト上で簡単にChatGPTの操作ができるしトークン数の節約をしながら記憶の半永久保存も簡単にできるくん ver.6.5.5 \n\nmade_by :Dai-H15  s1f102200828@iniad.org\n____________________\n")
    ...
    while True :
        #条件分岐
        print_raw = "有効" if raw_mode == True else "無効" 
        if error_openAI:
        ...
        #1行のみ質問を入力
        if input_type =="one":
            if (error_openAI or error_DeepL) == True:
